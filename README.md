...
Keep in mind that each entry is saved in a separate file. Therefore, storing a lot of atomic values like `double` may not be very practical. Conversely, saving large objects such as `String`s or `List<int>` is efficient. It's almost like writing directly to files, but without restrictions on key names.
